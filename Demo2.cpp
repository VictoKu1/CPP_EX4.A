...
void check_cure_discovery(Player &player) {
  ...
  cout << "  Four red cards, no research station: "
       << can_discover_cure(player, Color::Red) << endl;
...
int main() {
  ...
  }
  {
    ...
    check_cure_discovery(
        player); // should print: false false ***true*** false true [can find a
                 // cure with 4 red and 1 black card]
  }
  {
    Researcher player(board, City::Washington);
    check_cure_discovery(
        player); // should print: false false false ***true*** true [can find a
                 // cure without a research station]
  }
  {
    Scientist player(board, City::Washington, 4);
    check_cure_discovery(
        player); // should print: false ***true*** ***true*** false true  [can
                 // find a cure with only 4 red cards]
  }
}
